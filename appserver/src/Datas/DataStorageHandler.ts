...
function UpdateLocalVacation(year_month_day: string, type: number) {
    ...
    if (-1 != idx) vacationList.splice(idx, 1)
    idx = half_vacationList.indexOf(year_month_day)
    if (-1 != idx) half_vacationList.splice(idx, 1)
    idx = holidayWorkingList.indexOf(year_month_day)
    if (-1 != idx) holidayWorkingList.splice(idx, 1)
    if (1 == type) vacationList.push(year_month_day)
    else if (2 == type) half_vacationList.push(year_month_day)
    else if (3 == type) holidayWorkingList.push(year_month_day)
    UpdateAllViews()
}

export async function UpdateVacations(year_month_day: string, type: number) {
    ...
    if (member_id == null) {
        ...
        let datas: number[];
        if (null === dataStr) datas = Array.from({ length: 32 }, () => 0)
        else datas = JSON.parse(dataStr)
        ...
        try {
            const response = await axios.post('/api/request/update_vacation', {}, { params: { year, month, day, type } });
            UpdateLocalVacation(year_month_day, type)
        } catch(error) { console.log(error); }
    }
}

function UpdateLocalWorkingHour(working_hour_map: any, year_month: string) {
...
export async function UpdateWorkingHours(working_hour_map: any, year_month: string | null) {
    let member_id = getCookie('member_id')
    if (member_id == null) {
        ...
        if (year_month != null) UpdateLocalWorkingHour(working_hour_map, year_month)
        else location.href = '/'
    } else {
        try {
            const response = await axios.post('/api/request/set_working_hours', {}, { params: { map: JSON.stringify(working_hour_map) } });
            if (year_month != null) UpdateLocalWorkingHour(working_hour_map, year_month)
            else location.href = '/'
        } catch(error) { console.log(error); }
    }
...
export async function RequestWorkingInfos(year_month: string) {
    let member_id = getCookie('member_id')
    ...
    if (member_id == null) {
        ...
        if (datas) {
            ...
            for (let i = 0; i < 32; i++) {
                ...
                if (1 == work_types[i]) vacationList.push(date)
                else if (2 == work_types[i]) half_vacationList.push(date)
                else if (3 == work_types[i]) holidayWorkingList.push(date)
            }
        }
        const workingHourKey = "WorkingHour:" + String(year) + "-" + String(month).padStart(2, '0')
        datas = localStorage.getItem(workingHourKey)
        if (null === datas) working_hours = Array.from({ length: 32 }, () => 0)
        else working_hours = JSON.parse(datas)
        UpdateAllViews()
    } else {
        try {
            const response = await axios.post('/api/request/get_working_info', {}, { params: { year, month } });
            if (response.data['working_days'].length) {
                for (let i = 0; i < 32; i++) {
                    let date = String(year) + "-" + String(month).padStart(2, "0") + "-" + String(i).padStart(2, "0")
                    if (response.data['working_days'][i] == 1) vacationList.push(date)
                    else if (response.data['working_days'][i] == 2) half_vacationList.push(date)
                    else if (response.data['working_days'][i] == 3) holidayWorkingList.push(date)
                }
            }
            working_hours = response.data['working_hours'].length ? response.data['working_hours'] : Array.from({ length: 32 }, () => 0)
            UpdateAllViews()
        } catch(error) { console.log(error); }
    }
}
...
export function GetWorkingHour(): number[] { return working_hours; }
