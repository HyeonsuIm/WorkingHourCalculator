import { GetWorkingDayVal, GetCommonWorkingDayVal, GetVacationDayVal } from './CalendarAPI'
import { GetWorkingHour } from '../Datas/DataStorageHandler'
import { getSelectBase } from '../Layout/MainPageHandler'
import { UpdateAllViews } from './CalendarViewHandler'
...
let overtime_work_month: number = 0
let total_work_hour_month: number = 0

export function render_working_hour(year: number, month: number, day: number) {
    const viewYear = year
...
export function render_calculated_working_hour(year: number, month: number, day: number, isCurrentMonth: boolean) {
    const viewYear = year
    const viewMonth = month
...
export function GetWorkingDay(monthStartDay: Date, monthLastDay: Date, startDayOfWeek: number, today_date: number): [number, number] {
...
export function GetCommonWorkingDay(monthStartDay: Date, monthLastDay: Date, startDayOfWeek: number, today_date: number): [number, number] {
    let totalDayCnt = monthLastDay.getDate()
    let workingDayCnt = 0
    let workingDayCntAfterToday = 0
    for (let i = 1; i <= totalDayCnt; i++) {
...
export function GetVacations(monthStartDay: Date, monthLastDay: Date, today_date: number): [number, number] {
    let totalDayCnt = monthLastDay.getDate()
...
export function GetWorkingHours(totalDayCnt: number, totalWorkingDayCnt: number, vacationDayCnt: number): [number, number] {
...
export function MakeWorkingHourTable(maxWorkingHour: number, minWorkingHour: number) {
    SetRemainedWorkingHour("month_max_working_hour", maxWorkingHour)
    SetRemainedWorkingHour("month_min_working_hour", minWorkingHour)
}

export function SetAllRemainedWorkingHour(workingDayCntAfterToday: number, maxWorkingHour: number, minWorkingHour: number, remainedWorkingHour: number) {
    let remainedMaxWorkingHour = remainedWorkingHour
    let remainedPlanWorkingHour = remainedWorkingHour - working_hour_plan
    let expectWorkingHour = ((working_overpay_plan / payPerHour) + minWorkingHour) - (maxWorkingHour - remainedWorkingHour)
...
export function GetRemainedWorkingHour(maxWorkingHour: number, currentDay: number, currentWorkingDayVal: number, remainWorkingDayCnt: number): [number, number] {
...
export function GetRemainWorkingHour(maxWorkingHour: number, currentDay: number): [boolean, number] {
...
export function GetTotalWorkingHour(currentDay: number): number {
    let working_hours = GetWorkingHour()
    let total_miniute = 0
    for (let i = 0; i <= currentDay; i++) {
        total_miniute += working_hours[i]
...
export function GetRemainedWorkingStoreTime() {
    const remainedWorkingHourKey = "remainWorkingHour"
...
export function SetRemainedWorkingHour(elementId: string, value: number) {
...
export function updateWorkingOverpayPlan() {
    let overpayElement = document.getElementById('working_overpay') as HTMLInputElement
    if (overpayElement) {
...
export function UpdateRemainWorkingHour() {
    const remainedWorkingHourKey = "remainWorkingHour"
    let workingDoneElement = document.getElementById("working_done") as HTMLInputElement
    ...
    if (remainedWorkingHour != "") {
        if (-1 == remainedWorkingHour.indexOf(":")) {
            ...
            if (isNaN(remainedWorkingFloat)) {
                ...
                let remainedWorkingHourStr = remainedWorkingFloat.toFixed(0)
                let remainedWorkingMinute = (remainedWorkingFloat - parseInt(remainedWorkingHourStr)).toFixed(0)
                remainedWorkingHour = remainedWorkingHourStr + ":" + remainedWorkingMinute
...
export function renderOvernightPay() {
    ...
    element = document.getElementById('overtime_work_based_month') as HTMLElement
    if (overtime_work_month > 0) {
